...
/*--------------------------------------------------------------*/
function find_group_by_admin_id($id) {
   global $db;
   return find_by_sql("SELECT * FROM user_groups WHERE group_level != 0 AND admin_id='{$id}'");
}  
...
/*--------------------------------------------------------------*/
  function authenticate($username='', $password='') {
   ...
   return false;
    ...
    $username = $db->escape($username);
    $password = $db->escape($password);
    $sql  = sprintf("SELECT id,username,password,user_level,admin_id FROM users WHERE username ='%s' LIMIT 1", $username);
    $result = $db->query($sql);
    if($db->num_rows($result)){
      $user = $db->fetch_assoc($result);
      $password_request = sha1($password);
 ...
 /*--------------------------------------------------------------*/
  ...
  /*--------------------------------------------------------------*/
  /* Find all user related to the admin id
  /*--------------------------------------------------------------*/
  function find_by_admin_id($id){
      global $db;
      $results = array();
      $sql = "SELECT u.id,u.name,u.username,u.user_level,u.status,u.last_login,";
      $sql .="g.group_name ";
  ...
  function find_all_client(){
      ...
      $sql .="FROM users u ";
      $sql .="LEFT JOIN user_groups g ";
      $sql .="ON g.admin_id=u.admin_id WHERE u.user_level = 1 GROUP BY u.name  ORDER BY u.name ASC ";
      $result = find_by_sql($sql);
      return $result;
  }
...
/*--------------------------------------------------------------*/
function find_sale_by_dates($start_date,$end_date,$admin_id){
  global $db;
...
/*--------------------------------------------------------------*/
function  monthlySales($month,$admin_id){
  global $db;
  $sql  = "SELECT *,s.qty,";
  $sql .= " s.date,p.name,";
  $sql .= "SUM(p.sale_price * s.qty) AS total_saleing_price";
  ...
  $sql .= " WHERE MONTH(s.date)= '{$month}'  AND s.admin_id = '{$admin_id}'";
